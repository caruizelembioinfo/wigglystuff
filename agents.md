...
| PlottingCanvas | `wigglystuff.plotting_canvas.PlottingCanvas` | `width`, `height`, `x_label`, `y_label`, `selected_indices`, `_layers` | High-performance WebGL scatter canvas; use `add_scatter(x, y, color=..., colormap=...)` and `set_data()` for reactive updates |
